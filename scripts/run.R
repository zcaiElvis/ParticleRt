...
# covid <- covid[1:600,]
covid<- read_owid(covid_file_loc, inter = c(1:900))
...
normal_epifilter <- run_pfilter(rprocess_config = tran_normal_epifilter, dmeasure_config = meas_pois, data = covid, Np=2000)
epifilter_result <- as.data.frame(normal_epifilter)
plot(epifilter_result$pred.mean.x)

### Log normal ###
lnormal_filter <- run_pfilter(rprocess_config = tran_lognormal, dmeasure_config = meas_pois_lnorm, data=covid, Np=3000, verbose=FALSE)
